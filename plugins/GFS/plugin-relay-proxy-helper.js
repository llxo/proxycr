...
// 配置文件存储路径
const CONFIG_PATH = 'data/third/relay-proxy-helper'
const getConfigFile = (profileId) => `${CONFIG_PATH}/${profileId}.json`

/**
 * 加载配置
 */
const loadConfig = async (profileId, availableOutbounds) => {
  try {
    const configFile = getConfigFile(profileId)
    const exists = await Plugins.FileExists(configFile)
    if (!exists) return null

    const data = await Plugins.ReadFile(configFile)
    const config = JSON.parse(data)

    // 验证并过滤无效的出站
    if (config.relayChains && Array.isArray(config.relayChains)) {
      const validOutboundsSet = new Set(availableOutbounds)
      config.relayChains = config.relayChains
        .map((chain) => chain.filter((tag) => validOutboundsSet.has(tag)))
        .filter((chain) => chain.length > 0) // 移除空链
    }

    // 验证 globalOutSelected
    if (config.globalOutSelected && !availableOutbounds.includes(config.globalOutSelected)) {
      config.globalOutSelected = availableOutbounds[0] || null
    }
 ...
 * 保存配置
 */
const saveConfig = async (profileId, config) => {
  try {
    // 确保目录存在
    await Plugins.MakeDir(CONFIG_PATH).catch(() => {})
    
    const configFile = getConfigFile(profileId)
    await Plugins.WriteFile(configFile, JSON.stringify(config, null, 2))
  } catch (error) {
 ...
 * 清除配置
 */
const clearConfig = async (profileId) => {
  try {
    const configFile = getConfigFile(profileId)
    const exists = await Plugins.FileExists(configFile)
    if (exists) {
      await Plugins.RemoveFile(configFile)
    }
  } catch (error) {
  ...
  const component = {
    ...
    <div class="pr-8">
      <div class="py-8 flex items-center gap-4">
        ...
        <Button @click="resetConfig" type="text" title="清除保存的配置">重置</Button>
      </div>
    ...
    setup() {
      ...
      const loadSavedConfig = async () => {
        const profileId = window[Plugin.id].selectedProfileId
        if (!profileId) return

        const savedConfig = await loadConfig(profileId, window[Plugin.id].outbounds)
        if (savedConfig) {
          if (savedConfig.relayChains && savedConfig.relayChains.length > 0) {
            relayChains.value = savedConfig.relayChains
          }
          if (savedConfig.targetChainIndex !== undefined) {
            targetChainIndex.value = savedConfig.targetChainIndex
          }
          if (savedConfig.globalOutSelected) {
            globalOutSelected.value = savedConfig.globalOutSelected
          }
        }
      }

      // 保存配置（带防抖）
      const saveCurrentConfig = Plugins.debounce(() => {
        const profileId = window[Plugin.id].selectedProfileId
        if (!profileId) return

        const config = {
          relayChains: relayChains.value,
          targetChainIndex: targetChainIndex.value,
          globalOutSelected: globalOutSelected.value
        }
      ...
      // 监听其他状态变化并自动保存
      watch(targetChainIndex, saveCurrentConfig)
      watch(globalOutSelected, saveCurrentConfig)
      ...
      const resetConfig = async () => {
        ...
        await clearConfig(profileId)
        relayChains.value = [[]]
        targetChainIndex.value = 0
        globalOutSelected.value = outList[0]?.value ?? null
        Plugins.message.info('配置已重置')
      }
